...
#looper.asset-ui #library{grid-template-columns:repeat(3,minmax(0,1fr))!important;grid-template-rows:1fr!important;gap:1.15%!important}
#looper.asset-ui #library::before{display:none!important}
#looper.asset-ui #library .cassetteRackColumn{display:grid!important;grid-template-rows:repeat(3,minmax(0,1fr))!important;gap:1.7%!important;min-width:0!important;min-height:0!important}
#looper.asset-ui #library .cassetteRackColumn.asset-page-hidden{display:none!important}
...
#looper.asset-ui #library .cassetteRackSlot::after{content:"";position:absolute;z-index:0;left:30%;top:7%;width:68%;height:86%;background:rgba(8,7,5,.965);pointer-events:none}
...
#looper.asset-ui .asset-cassette-label-readout{left:39.1%!important;top:13.55%!important;width:25.7%!important;height:3.55%!important;font-size:clamp(7px,1.05vw,17px)!important}
#looper.asset-ui .asset-header-state-readout{left:5.25%!important;top:12.65%!important;width:8.2%!important;height:2.6%!important;background:#090705!important;font-size:clamp(6px,.8vw,13px)!important}
...
function installAssetLibraryPager(looper){
  ...
  let page=0;
  const paint=()=>{
    const columns=[...library.querySelectorAll(".cassetteRackColumn")];
    const pages=Math.max(1,Math.ceil(columns.length/3));
    page=Math.max(0,Math.min(page,pages-1));
    columns.forEach((column,index)=>column.classList.toggle("asset-page-hidden",Math.floor(index/3)!==page));
    library.querySelectorAll(".cassetteRackSlot").forEach(slot=>slot.classList.toggle("asset-slot-empty",!slot.querySelector(".track")));
    readout.textContent=`${page+1} / ${pages}`;
    prev.disabled=pages<=1;next.disabled=pages<=1;
  };
  prev.onclick=()=>{page=Math.max(0,page-1);paint();};
  next.onclick=()=>{const pages=Math.max(1,Math.ceil(library.querySelectorAll(".cassetteRackColumn").length/3));page=Math.min(pages-1,page+1);paint();};
...
function installAssetSpeedControl(){
  ...
  let speedLevel=0,loopBaseUnits=typeof tapeCounterUnits==="number"?tapeCounterUnits:0;
  const paintSpeed=()=>{
  ...
  const paintLoops=()=>{
    if(!deckBuffer||typeof tapeCounterUnits!=="number"){readouts.loops.textContent="0 / 8";return;}
  ...
  const applySpeedLevel=level=>{
    ...
    autoLooperEnabledState=false;autoLooperModeIndex=0;autoLooperSpeedPercent=100+speedLevel;
    stopAutoLooperProgress();
    if(deckSource)deckSource.playbackRate.value=deckRate();
    refreshCassetteUI();paintSpeed();
  };
  button.onclick=event=>{event.stopPropagation();applySpeedLevel((speedLevel+1)%6);};
  if(resetButton){
    const nativeReset=resetButton.onclick;
    resetButton.onclick=event=>{if(typeof nativeReset==="function")nativeReset.call(resetButton,event);loopBaseUnits=typeof tapeCounterUnits==="number"?tapeCounterUnits:0;applySpeedLevel(0);paintLoops();};
  }
  const trackName=document.getElementById("cassetteBeatName");
  if(trackName)new MutationObserver(()=>{loopBaseUnits=typeof tapeCounterUnits==="number"?tapeCounterUnits:0;if(autoLooperSpeedPercent===100&&speedLevel!==0)applySpeedLevel(0);paintLoops();}).observe(trackName,{childList:true,subtree:true,characterData:true});
  setInterval(paintLoops,100);paintSpeed();paintLoops();
}

loadLooperAsset();
window.addEventListener("load",()=>installAssetSpeedControl(),{once:true});

document.querySelectorAll("[data-range-knob]").forEach(knob=>{
  const input=document.getElementById(knob.dataset.rangeKnob);if(!input)return;
  const valueDescriptor=Object.getOwnPropertyDescriptor(HTMLInputElement.prototype,"value");
  const sync=()=>{const min=Number(input.min)||0,max=Number(input.max)||100,value=Number(input.value)||0,pct=max===min?0:(value-min)/(max-min)*100;knob.style.setProperty("--knob-pct",String(Math.max(0,Math.min(100,pct))));};
  input.addEventListener("input",sync);
  if(valueDescriptor?.get&&valueDescriptor?.set)Object.defineProperty(input,"value",{configurable:true,get(){return valueDescriptor.get.call(this);},set(value){valueDescriptor.set.call(this,value);sync();}});
  sync();
});

...
assert.equal(evaluate("DEFAULT_BEATS.length"),3);
assert.equal(evaluate("DEFAULT_BEATS.every(row=>row.url.startsWith('./assets/beats/'))"),true);
...
assert.equal(evaluate("isBundledBeat(DEFAULT_BEATS[0])"),true);
...
assert.equal(evaluate("visibleLibraryRows([{name:'User',source:'user'},{name:'Included',source:'bundled'}],'','name')[0].source"),"bundled");
...
blob.arrayBuffer().then(bytes=>{
  ...
  console.log("OK: core unit tests — utilities, tape counter, bundled beats and WAV export");
}).catch(error=>{
  console.error(error);
  process.exitCode=1;
});
